...
def nested_all_basic(model_name, X, y, leftout, p_threshold, hyper_range, inner_folds, covariate, control_method, random_seed, if_permutation, permutation_seed):

    """Sub-function that will be used in nested_all_model for parallel computing
      - model_name: specify the desired algorithm, such as svr, ols, lasso, ridge
      ...
      - hyper_range: a list of alternative values for hyperparameter tuning (e.g., C for SVR, alpha for ridge regression)
    ...
    for hyper_para in hyper_range:
        ...
        # create a model
        if model_name == "svr":
            model = LinearSVR(C=hyper_para, random_state=random_seed)
        elif model_name == "ols":
            model = linear_model.LinearRegression()
        elif model_name == "lasso":
            model = linear_model.Lasso(alpha=hyper_para, random_state=random_seed)
        elif model_name == "ridge":
            model = linear_model.Ridge(alpha=hyper_para, random_state=random_seed)
        else:
            break
        ...
        temp_results.loc[i, 'hyper_para'] = hyper_para
    ...
    best_hyper_para = temp_results.loc[0, 'hyper_para']

    # use the whole training set to predict on test set (i.e., y for the subject been left out)
    if model_name == "svr":
        best_model = LinearSVR(C=best_hyper_para, random_state=random_seed)
    elif model_name == "ols":
        best_model = linear_model.LinearRegression()
    elif model_name == "lasso":
        best_model = linear_model.Lasso(alpha=best_hyper_para, random_state=random_seed)
    elif model_name == "ridge":
        best_model = linear_model.Ridge(alpha=best_hyper_para, random_state=random_seed)
    else:
        best_model = "error"
...
def nested_all_model(model_name, X, y, covariate, p_threshold, hyper_range, inner_folds, control_method, control_threshold, random_seed=0, parallel_quantity=-1, if_permutation=False, permutation_seed=0):
    ...
    Parallel(n_jobs=parallel_quantity, backend="loky")(delayed(nested_all_basic)(model_name, X, y, leftout, p_threshold, hyper_range, inner_folds, covariate, control_method, random_seed, if_permutation, permutation_seed)
    ...
    df_results['y'] = y
    df_results.to_csv(f"output/nest_{model_name}_predictions_{int(time.time())}.csv", index=False)

    # evaluate model performance
    [r, p] = stats.pearsonr(y_predictions, y)
    print(f'{model_name} model performance: r = {r}, p = {p}, MAE={mean_absolute_error(y, y_predictions)}')
